...
In the exercise below, use the `labs()` function as demonstrated in [Section 8.1](https://ggplot2-book.org/annotations.html#titles) of the textbook to add the following titles shown below on the solution plot:
...
In the exercise below, use the function & argument demonstrated in [section 10.3.2](https://ggplot2-book.org/scale-position.html#scale-labels) of the textbook to change the labels of the iris species to their common names (per Wikipedia) instead of their taxonomic names:
...
In the exercise below, use one of the functions demonstrated in [section 10.3.3](https://ggplot2-book.org/scale-position.html#guide_axis) of the textbook to rotate the axis labels for the state names by 45 degrees counter-clockwise. (The labels should appear to be right-justified to the axis.) 
...
Now, use another function demonstrated in section 10.3.3 to "dodge" the axis labels by a factor of 5 so that state name labels are jittered into 5 rows.
...
In the exercise below, use the function demonstrated in the [forcats package vignette: Ordering by another variable](https://forcats.tidyverse.org/articles/forcats.html#ordering-by-another-variable-1) to reorder the states along the x-axis by area from smallest to largest.
...
In the exercise below, use the `ggplot2` function `expand_limits()` to show 0 on the y-axis, feferring to the function [docs](https://ggplot2.tidyverse.org/reference/expand_limits.html) for more info on what arguments to set.
...
In the exercise below, use the "zooming" limit adjustment function (NOT the one that will trim data if they fall outside of the limits!) to change the y-axis bounds to 0 on the lower end and 8 on the higher end, referring to [section 10.1.2](https://ggplot2-book.org/scale-position.html#oob) of the textbook for examples.
...
To address this, in the exercise below, use one of the convenience functions demonstrated in [section 10.1.9](https://ggplot2-book.org/scale-position.html#scale-transformation) of the textbook to show x-axis tick labels on the log-10 scale.
...
First, in the exercise below, use `scales::breaks_width()` to set the major breaks to occur on the 10s, as shown in [section 10.1.5](https://ggplot2-book.org/scale-position.html#break-functions) of the textbook.
...
Refer to [section 11.3.1](https://ggplot2-book.org/scale-colour.html#brewer-scales) of the textbook for examples.
...
Refer to [section 11.2.1](https://ggplot2-book.org/scale-colour.html#particular-palettes) of the textbook for examples.
...
Refer to [section 11.3.3](https://ggplot2-book.org/scale-colour.html#manual-scales) of the textbook for examples.
...
Refer to some of the examples in the later half of [section 8.2](https://ggplot2-book.org/annotations.html#text-labels) of the textbook to see how `geom_text()` is used.
...
Refer to the function [docs](https://ggplot2.tidyverse.org/reference/geom_text.html) to see what the necessary arguments are called.
...
Refer to some of the examples in [section 8.3](https://ggplot2-book.org/annotations.html#custom-annotations) to see how to specify the arguments for a text annotation.

```{r make-plot-us-states-ang-reo-lab-ann}
...
Refer to some of the examples in [section 8.2](https://ggplot2-book.org/annotations.html#text-labels) of the textbook, as well as the function docs for `geom_text()`, to identify the correct arguments to set. Note that even though you are calling `annotate()`, the arguments eventually get passed down to `geom_text()` when `ggplot2` draws the annotation, which is why you have to look in the `geom_text()` docs for the function argument info.
...
Refer to [section 18.3](https://ggplot2-book.org/polishing.html#modifying-theme-components) for examples of how to use the various arguments of `theme()` to adjust their respective theme elements, to the [docs](https://ggplot2.tidyverse.org/reference/theme.html) for `theme()` to identify which theme element should be modified, and to the [docs](https://ggplot2.tidyverse.org/reference/element.html) for `element_text()` for specific argument names.
...
In the exercise below, change all the text to Comic Sans MS font (again), _and_ change the plot's subtitle to appear in **bold** font face.
...
In the exercise below, use `rel()` to make the axis tick labels (on both axes) appear 2x as big as its usual "parent" size. (You can refer to the function [docs](https://ggplot2.tidyverse.org/reference/element.html) for `rel()`, but they don't say much, as `rel()` only takes one argument and performs one operation on it.)
...
Please note: Text annotations on a plot (as demonstrated earlier in the worksheet) are not included in text theme elements, so if you have text annotations on a plot and want them to appear the same as title and axis text, you will need to set their appearance arguments separately.
...
In the exercise below, change the color of _all_ grid lines on the plot to "deeppink". Refer  to the [docs](https://ggplot2.tidyverse.org/reference/element.html) for `element_line()` for specific argument names.
...
In the exercise below, change the the border of the panel background (the panel containing the data) to the color "navyblue", and use `rel()` to set the border thickness to 2 times the parent size.

Hint: Yes, panel border is still set using `element_rect()`, because it's the outline of the panel rectangle. Remember the difference between the `fill` and `color` arguments!
