...
* When a base plot variable is provided in an exercise chunk, you _will not_ need to edit the base plot to complete that exercise. You will only need to add new layers
* The code that creates base plots will always be displayed in a read-only chunk for your reference
* When raw `ggplot2` code is provided in an exercise chunk, you _will_ need to edit the base plot to complete that exercise. You may _also_ need to add new layers, but not necessarily
...
To illustrate this more clearly, let's look at a different dataset now: the area (in square miles) of the 50 US states. This dataset helps illustrate the benefits of the next few techniques, in particular because it has a _lot_ of levels (50!) of a discrete variable.
...
As you can see, for these data, dodging still doesn't fully stop the state names from plotting on top of one another. For this reason, I usually prefer rotating axis labels over dodging them, but either might be the best choice depending on the plot. We'll use counterclockwise-rotated axis labels for the rest of the plots in this section.
